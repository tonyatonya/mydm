	...
	if(!$_POST){
		...
		try{
			...
			if($_POST["cmd"] == "loadData"){
				...
				$sql = "select count(id) as data_count from d_product_categories where id <> 0 ".$cause;
				...
				$i = $start_limit;
				$sql = "select * from d_product_categories where id <> 0 ".$cause." order by id desc LIMIT ".$start_limit.",".$page_size;
				if(!$db->execute($sql))
					throw new Exception("Operation Error");
				while($db->read()){
					$i += 1;
				...
				$sql = "delete from d_product_categories where id='".$_POST['id']."' ";
