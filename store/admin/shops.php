		...
		try{
			...
			if($_POST["cmd"] == "loadData"){
				...
				if(isset($_POST[search_text])){
					...
					$cause .= "and  (
										A.name LIKE '%".$search_text."%' or 
										B.name LIKE '%".$search_text."%' 
										)";
				}
				
				$sql = "select count(A.id) as data_count from d_products A left outer join d_product_categories B on A.category_id = B.id where A.id <> 0 ".$cause;
				...
				$i = $start_limit;
				//$sql = "select * from d_products where id <> 0 ".$cause." order by id desc LIMIT ".$start_limit.",".$page_size;
				$sql = "select A.*, B.name as collection_name from d_products A left outer join d_product_categories B on A.category_id = B.id where A.id <> 0  ".$cause." order by A.id desc LIMIT ".$start_limit.",".$page_size;
				...
				while($db->read()){
					...
					$content_data .= "<tr  cellpadding='2'>
								...
								<td >".$db->result['collection_name']."</td>
				...
				$sql = "delete from d_products where id='".$_POST['id']."' ";
