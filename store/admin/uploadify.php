...
$targetFolder = "../images/products/product-detail/".$_POST['p_id']."/";
	...
	if (!empty($_FILES) && $_POST['token'] == $verifyToken) {
		...
		if($_FILES['Filedata']["size"] > 6 * 1048576 ){	//1048576B = 1MB
			...
			if(!move_uploaded_file($tempFile,$targetFile)){
				...
				$i= 1;
				$count_img=1;
				$sql="select * from sto_product_collection where p_id = '".$_POST['p_id']. "' order by seq desc";
				if(!$db->execute($sql))
...
//					$row =  count($db->result);
				...
				if($count_img >= 6){
					$data["what"] = "error";
					$data["result_text"] = "Picture Gallary Maximum is 6";
				}else {
