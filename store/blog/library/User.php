...
//--------------------------------------------------------------------------------------------------------
		...
		public function login(&$param, $user_type){
			...
			$sql = "select * from m_user  where username = '".$param['username']."' and password = '".md5($param['password'])."' ";
			$db->execute($sql);
			if(!$db->read()){
				...
				if($user_type != "" and $user_type != $db->result[user_type]){
					$this->err = "ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง";
					return false;
				}
					
				$this->keep_login($db->result[username],$db->result[password],$db->result[name]." ".$db->result[surname],$db->result['email'],$db->result[user_type],$db->result[company_id]);
				
				$sql = "update m_user set last_access_time = NOW() where username = '".$param['username']."' ";
				if(!$db->execute($sql))
					return false;
